...
public sealed class RtpRtcpTests
{
    ...
    public void SenderReport_WriteThenParse_PreservesNtpRtpMapping()
    {
        ...
        var sr = new RtcpSenderReport(
            0x11112222,
            NtpTimestamp: 0xE1E2E3E4_F1F2F3F4,
            RtpTimestamp: 90000,
            SenderPacketCount: 1234,
            SenderOctetCount: 567890,
            blocks
        );
        ...
        Assert.IsTrue(
            RtcpSenderReport.TryParse(buffer.AsSpan(0, length), out RtcpSenderReport parsed)
        );
    ...
    {
        var blocks = new RtcpReportBlock[] { new(0xDEADBEEF, 0, 0, 12345, 7, 0, 0) };
        ...
        Assert.IsTrue(
            RtcpReceiverReport.TryParse(buffer.AsSpan(0, length), out RtcpReceiverReport parsed)
        );
    ...
    public void Compound_SenderReportFollowedByReceiverReport_BothParse()
    {
        ...
        Assert.IsTrue(
            RtcpSenderReport.TryParse(buffer.AsSpan(0, n), out RtcpSenderReport parsedSr)
        );
        Assert.AreEqual(0xAAAA_BBBB_CCCC_DDDDul, parsedSr.NtpTimestamp);
        Assert.IsTrue(
            RtcpReceiverReport.TryParse(buffer.AsSpan(0, n), out RtcpReceiverReport parsedRr)
        );
