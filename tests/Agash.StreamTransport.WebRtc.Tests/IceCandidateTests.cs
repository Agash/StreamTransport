...
public sealed class IceCandidateTests
{
    ...
    public void ComputePairPriority_MatchesRfc8445Formula()
    {
        ...
        Assert.AreEqual(
            expected,
            IceCandidate.ComputePairPriority(d, g) - 1,
            "the G>D tie-bit distinguishes the roles"
        );
    }
    ...
    public void ToSdp_ThenTryParse_RoundTripsHostCandidate()
    {
        var candidate = new IceCandidate(
            ...
            1,
            2130706431,
            new IPEndPoint(IPAddress.Parse("192.168.1.5"), 51556),
            IceCandidateKind.Host
        );
    ...
    {
        const string line =
            "candidate:3 1 udp 1694498815 203.0.113.7 54321 typ srflx raddr 192.168.1.5 rport 51556";
    ...
    {
        Assert.IsFalse(
            IceCandidate.TryParse("candidate:1 1 tcp 2130706431 192.168.1.5 9 typ host", out _)
        );
    ...
    public void Credentials_Generate_MeetsEntropyAndCharset()
    {
        ...
        Assert.IsTrue(
            (a.UsernameFragment + a.Password).All(static c =>
