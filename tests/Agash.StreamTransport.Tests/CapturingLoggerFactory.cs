...
internal sealed class CapturingLoggerFactory : ILoggerFactory
{
    ...
    public ILogger CreateLogger(string categoryName) =>
        new CapturingLogger(categoryName, _entries, _clock);
    ...
    public string Dump() =>
        _entries.IsEmpty ? "(no log entries)" : string.Join(Environment.NewLine, _entries);
    ...
    ) : ILogger
    {
        ...
        )
        {
            ...
            string line =
                $"{clock.ElapsedMilliseconds, 6}ms [{logLevel.ToString()[0]}] {_shortCategory}: {formatter(state, exception)}";
