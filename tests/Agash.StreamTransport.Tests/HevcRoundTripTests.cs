...
public sealed class HevcRoundTripTests
{
    ...
    public void EncodeThenDecode_Hevc_RecoversFrameDimensions()
    {
        ...
        {
            Assert.Inconclusive(
                "No bundled FFmpeg native build found; skipping HEVC round-trip test."
            );
            return;
        }
        ...
        using (encoder)
        {
            ...
            using IVideoDecoderBackend decoder =
                encoderName == "hevc_vaapi" ? new VaapiVideoDecoder() : new HevcDecoder();
                ...
                for (int frame = 0; frame < 30 && !decodedAny; frame++)
                {
                    ...
                    if (decoder.TryDecode(accessUnit, 0, 0, out VideoFrame decoded, out _))
                    {
                        Assert.AreEqual(
                            width,
                            decoded.Width,
                            "Decoded width should match the encoded frame."
                        );
                        Assert.AreEqual(
                            height,
                            decoded.Height,
                            "Decoded height should match the encoded frame."
                        );
                        Assert.AreEqual(
                            width * height * 3 / 2,
                            decoded.Pixels.Length,
                            "Decoded buffer should be a full 4:2:0 frame."
                        );
                        decodedAny = true;
                    }
                }
            ...
            Assert.IsTrue(
                decodedAny,
                "Expected at least one decoded frame from the HEVC round-trip."
            );
            ...
            )
            {
                Assert.AreEqual(
                    "hevc_cuvid",
                    hevc.DecoderName,
                    "Hardware NVDEC decoder should be selected when available."
                );
            }
        }
    }
