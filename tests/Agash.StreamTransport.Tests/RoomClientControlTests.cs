...
public sealed class RoomClientControlTests
{
    ...
    public async Task ControlMessage_RoundTripsThroughRouter_WithFromStamped()
    {
        ...
        var received = new TaskCompletionSource<PeerControlMessage>(
        ...
        Assert.AreEqual(
            publisher.Self,
            got.From,
            "the router stamps From from the sending session."
        );
    }
    ...
    private sealed class InMemoryClientLink : IDuplexSignalingTransport
    {
        ...
        public InMemoryClientLink(SignalingRouter router) =>
            _session = router.Connect(new RouterSink(this));
        ...
        private Task Deliver(SignalingMessage message) =>
