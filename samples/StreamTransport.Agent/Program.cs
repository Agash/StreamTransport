...
if (args.Length > 0 && args[0].Equals("selftest", StringComparison.OrdinalIgnoreCase))
{
    ...
    if (
        args.Length > 1
        && args[1].Equals("pwdmabuf", StringComparison.OrdinalIgnoreCase)
    ...
    AnsiConsole.MarkupLine(
        "[yellow]selftest is available on macOS (Syphon) and Windows (Spout) only.[/]"
    );
    return 1;
#endif
}
...
bool verbose = Array.Exists(
    args,
    a => string.Equals(a, "--verbose", StringComparison.OrdinalIgnoreCase)
);
...
builder.Logging.AddProvider(
    new SpectreLoggerProvider(verbose ? LogLevel.Debug : LogLevel.Information)
);
...
networkMonitor.NetworksChanged += paths =>
    networkLogger.LogInformation(
...
try
{
    return config.Role == PeerRole.Publisher
        ? await host
            .Services.GetRequiredService<Publish>()
            .RunAsync(config, transport, shutdown.Token)
        : await host
            .Services.GetRequiredService<Subscribe>()
            .RunAsync(config, transport, shutdown.Token);
}
...
)
{
    public static AgentConfig? FromArgs(string[] args)
    {
        ...
        PeerRole role =
            args[0].Equals("send", StringComparison.OrdinalIgnoreCase) ? PeerRole.Publisher
            : args[0].Equals("receive", StringComparison.OrdinalIgnoreCase) ? PeerRole.Subscriber
            : throw new ArgumentException(
                $"first argument must be 'send' or 'receive', got '{args[0]}'."
            );
        ...
        for (int i = 1; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "--devtunnel":
                    ...
                    break;
                case "--source":
                    source = Enum.Parse<VideoSourceKind>(args[++i], ignoreCase: true);
                    break;
                case "--video-device":
                    videoDevice = args[++i];
                ...
                case "--spout-sender":
                    spoutSender = args[++i];
                    source = VideoSourceKind.Spout;
                    break;
                case "--syphon-server":
                    syphonServer = args[++i];
                    source = VideoSourceKind.Syphon;
                    break;
                case "--publish-spout":
                    publishSpout = args[++i];
                    break;
                case "--publish-syphon":
                    publishSyphon = args[++i];
                    break;
                case "--publish-pipewire":
                    publishPipeWire = args[++i];
                    break;
                case "--encoder":
                    encoder = args[++i];
                    break;
                ...
                case "--interface":
                    ...
                    break;
                case "--verbose":
                    break; // handled before host build; ignore here.
                default:
        ...
        {
            throw new ArgumentException(
                "--host/--devtunnel is only valid for 'send' (the sender hosts signaling)."
            );
        }
        ...
        if (!host)
        {
            ...
            {
                throw new ArgumentException(
                    "--relay <ws-url> is required (or use --host / --devtunnel for send)."
                );
            }
        }
        ...
        room ??=
            role == PeerRole.Publisher
                ? RandomRoom()
                : throw new ArgumentException("--room <code> is required for receive.");
    ...
    private static MediaProfile ParseProfile(string value) =>
        value.ToLowerInvariant() switch
        {
            ...
            _ => throw new ArgumentException(
                $"unknown profile '{value}' (interactive|screenshare|irl)."
            ),
        };
...
internal static class Interactive
{
    public static AgentConfig Prompt()
    {
        ...
        if (publisher)
        {
            ...
            if (mode.StartsWith("connect", StringComparison.Ordinal))
            {
                relay = new Uri(
                    AnsiConsole.Prompt(
                        new TextPrompt<string>("Relay WebSocket URL:").DefaultValue(
            ...
            string choice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    ...
                    .AddChoices(
                        ...
                        $"camera ({CaptureBackend.VideoInputFormat})"
                    )
            );
            if (choice.StartsWith("camera", StringComparison.Ordinal))
            {
                source = VideoSourceKind.Camera;
                videoDevice = AnsiConsole.Prompt(
                    new TextPrompt<string>("Camera device:").DefaultValue(DefaultVideoDevice())
                );
                audioDevice = AnsiConsole.Prompt(
                    new TextPrompt<string>("Microphone device (blank for none):").AllowEmpty()
                );
            }
        }
        ...
        string room = publisher
            ? AnsiConsole.Prompt(
                new TextPrompt<string>("Room code:").DefaultValue(AgentConfig.RandomRoom())
            )
            : AnsiConsole.Prompt(new TextPrompt<string>("Room code to join:"));

        bool audio =
            source == VideoSourceKind.Synthetic
            || !string.IsNullOrWhiteSpace(audioDevice)
            || !publisher;
        return new AgentConfig(
    ...
    private static string DefaultVideoDevice() =>
        OperatingSystem.IsWindows() ? "Integrated Camera"
        : OperatingSystem.IsMacOS() ? "0"
        : "/dev/video0";
}
